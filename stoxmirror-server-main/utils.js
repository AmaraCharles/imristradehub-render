...
//     to: "support@imristradehub.com ", // list of receivers
...
const sendTicketEmail = async ({ name, complaint,email }) => {
  ...
  let info = await transporter.sendMail({
    from: `"imristradehub Team" <${process.env.EMAIL_USER}>`, // sender address
    ...
    html: `
      ...
      <body>
        <div class="email-container">
          <div class="header">
            <img src="cid:logo" alt="imristradehub Logo">
          ...
          <div class="footer">
            <p>
              If you did not sign up for imristradehub, please ignore this email or
...
const sendAdminTicketEmail = async ({ name, complaint,email }) => {
  ...
  let info = await transporter.sendMail({
    from: `"imristradehub Team" <${process.env.EMAIL_USER}>`, // sender address
    to: "support@imristradehub.com", // recipient address
    subject: "Ticket Registered!", // subject line
    html: `
      <html>
      <head>
...
const sendForgotPasswordEmail = async (email) => {
  ...
  let info = await transporter.sendMail({
    ...
    <a href="https://imristradehub.com/reset-password">
    reset password
    </p>
...
const sendVerificationEmail = async ({ from, url }) => {
  ...
  let info = await transporter.sendMail({
    ...
    <p>${from} just verified his imristradehub Team Identity
    </p>
...
const sendWelcomeEmail = async ({ to, otp }) => {
  ...
  let info = await transporter.sendMail({
    ...
    subject: "Welcome to imristradehub!", // subject line
    html: `
      <html>
      <head>
        <style>
      ...
      <body>
        <div class="email-container">
          ...
          <div class="content">
            <h2>Welcome to imristradehub!</h2>
            <p>
              Thank you for joining imristradehub! We're excited to have you on board.
    ...
    attachments: [
      ...
      },
      {
        ...
        cid: 'logos', // This ID matches the 'cid' used in the HTML
      },
    ],
  });
...
//'<a href="https://imristradehub.com/imristradehub.com/verified.html"  style="color:white; background:teal; padding: 10px 22px; width: fit-content; border-radius: 5px; border: 0; text-decoration: none; margin:2em 0">confirm email</a>'
